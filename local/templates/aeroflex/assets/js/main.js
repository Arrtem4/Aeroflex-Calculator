$(document).ready(function () {
    ...
    $("._sliderAdvantages").on(
        "beforeChange",
        function (event, slick, currentSlide, nextSlide) {
            $("._goToSlide").removeClass("active");
            $("._goToSlide[data-item=" + (nextSlide + 1) + "]").addClass(
                "active"
            );
        }
    );
    ...
    $("._selectTab").on("click", function () {
        ...
        $(this).addClass("active");
        $(parent)
            .find(".info__block[data-item=" + attr + "]")
            .addClass("active");
    });
    ...
    $("._chooseFile").on("click", function () {
        ...
        $(parent)
            ...
            .each(function (item, elem) {
                if ($(elem).prop("checked")) {
                    counter++;
                }
            });
        $(parent)
            .find(".download__archive .download__button")
            .text("Скачать " + counter + " файла");
        $(parent)
            .find(".download__archive .download__count")
            .text("Выбрано " + counter + " файла");
    });
    ...
    $("._projectSlider_v2").on(
        "beforeChange",
        function (event, slick, currentSlide, nextSlide) {
            $("._block_2 .project__more").removeClass("active");
            $(
                "._block_2 .project__more[data-item=" + (nextSlide + 1) + "]"
            ).addClass("active");
        }
    );
    ...
    $("._projectSlider_v3").on(
        "beforeChange",
        function (event, slick, currentSlide, nextSlide) {
            $("._block_3 .project__more").removeClass("active");
            $(
                "._block_3 .project__more[data-item=" + (nextSlide + 1) + "]"
            ).addClass("active");
        }
    );
...
function init() {
    ...
    var myMap = new ymaps.Map("map", {
        ...
        myPlacemark = new ymaps.Placemark(
            [58.00943032859066, 56.16439437743621],
            {
