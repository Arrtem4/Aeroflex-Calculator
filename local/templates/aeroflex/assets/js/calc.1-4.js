$(function () {
    ...
    $('[name="region"]').on("change", function () {
        ...
        if ($indoor.val() === "open") {
            ...
            const $gasMovingTemperature = $calc.find(
                '[name="gas-moving-temperature"]'
            );
            const gasMovingTemperature = Number(
                $gasMovingTemperature.val().replace(",", ".")
            );
    ...
    $(".temperature_out").on("change", function () {
        ...
        const gasMovingTemperature = parseFloat(
            $gasMovingTemperature.val().replace(/,/, ".")
        );

        if (
            isNaN(currentValue) ||
            isNaN(gasMovingTemperature) ||
    ...
    $('[name="gas-pipe-type"]').on("change", function () {
        ...
        const $rectangularPipeDiameterOutRow = $calc.find(
            ".rectangular-pipe-diameter-out"
        );
        const $rectangularPipeDiameterInRow = $calc.find(
            ".rectangular-pipe-diameter-in"
        );
    ...
    $('[name="coolant-type"]').on("change", function () {
        ...
        const $gasPipeInnerDiameter = $calc.find(
            '[name="gas-pipe-inner-diameter"]'
        );
        const $heatTransferCoefficient = $calc.find(
            '[name="heat-transfer-coefficient"]'
        );

        const gasSpeed = parseFloat($gasSpeed.val().replace(/,/, "."));
        const gasPipeInnerDiameter = parseFloat(
            $gasPipeInnerDiameter.val().replace(/,/, ".")
        );
        const diameterIn = parseFloat($diameterIn.val().replace(/,/, "."));

        const diameterInRes =
            $gasPipeType.val() === "rectangular"
                ? gasPipeInnerDiameter
                : diameterIn;

        const $gasThermalConductivity = $calc.find(
            '[name="gas-thermal-conductivity"]'
        );
        const $gasKinematicViscosity = $calc.find(
            '[name="gas-kinematic-viscosity"]'
        );
        const $gasThermalDiffusivity = $calc.find(
            '[name="gas-thermal-diffusivity"]'
        );
        ...
        if ($(this).val() === "air") {
            if (!isNaN(gasMovingTemperature)) {
                ...
                } = AeroflexCalc.getGasProperties(
                    parseFloat($gasMovingTemperature.val().replace(/,/, "."))
                );
                ...
                $calc
                    .find(
                        '[name="gas-thermal-diffusivity"], [name="gas-kinematic-viscosity"], [name="gas-thermal-conductivity"]'
                    )
                    .prop("readonly", true);
                $calc
                    .find(
    ...
    $('[name="gas-moving-temperature"]').on("change", function () {
        ...
        if ($(this).val() && $gasMovingHumidity.val()) {
            $calc
                .find('[name="dew-point-temperature"]')
                .val(
                    AeroflexCalc.getGasDewPointTemperature(
                        parseFloat($(this).val().replace(/,/, ".")),
                        +$gasMovingHumidity.val()
                    )
                );
        } else {
    ...
    $('[name="gas-thermal-conductivity"]').on("change", function () {
        ...
        const gasThermalDiffusivity = parseFloat(
            $gasThermalDiffusivity.val().replace(/,/, ".")
        );
        const gasKinematicViscosity = parseFloat(
            $gasKinematicViscosity.val().replace(/,/, ".")
        );
        const currentValue = parseFloat($(this).val().replace(/,/, "."));
    ...
    $('[name="gas-kinematic-viscosity"]').on("change", function () {
        ...
        const gasThermalConductivity = parseFloat(
            $gasThermalConductivity.val().replace(/,/, ".")
        );
        const gasThermalDiffusivity = parseFloat(
            $gasThermalDiffusivity.val().replace(/,/, ".")
        );
    ...
    $('[name="gas-pipe-width"]').on("change", function () {
        ...
        const $gasPipeOuterDiameter = $calc.find(
            '[name="gas-pipe-outer-diameter"]'
        );

        if ($(this).val() && $gasPipeHeight.val()) {
            const diameterOuter = AeroflexCalc.getRectangularPipeDiameter(
                parseFloat($(this).val().replace(/,/, ".")),
                parseFloat($gasPipeHeight.val().replace(/,/, "."))
            );
    ...
    $('[name="gas-pipe-height"]').on("change", function () {
        ...
        if ($(this).val() && $gasPipeWidth.val()) {
            const diameterOuter =
                AeroflexCalc.getRectangularPipeDiameter(
                    parseFloat($gasPipeWidth.val().replace(/,/, ".")),
                    parseFloat($(this).val().replace(/,/, "."))
                ) * 1000;
    ...
    $(".calc_test ._result").on("click", function () {
        let $calc = $(this).closest(".calc_test"),
            ...
            $gasPipeOuterDiameter = $calc.find(
                '[name="gas-pipe-outer-diameter"]'
            ), // внешний (приведенный) диаметр газохода
            $gasPipeInnerDiameter = $calc.find(
                '[name="gas-pipe-inner-diameter"]'
            ), // внутренний (приведенный) диаметр газохода
            ...
            $gasMovingTemperature = $calc.find(
                '[name="gas-moving-temperature"]'
            ), // температура транспортируемого газа(теплоносителя)
            ...
            $gasThermalConductivity = $calc.find(
                '[name="gas-thermal-conductivity"]'
            ), // теплопроводность газа
            $gasKinematicViscosity = $calc.find(
                '[name="gas-kinematic-viscosity"]'
            ), // кинематическая вязкость газа(теплоносителя)
            $gasThermalDiffusivity = $calc.find(
                '[name="gas-thermal-diffusivity"]'
            ); // Температуропроводность газа
        ($heatTransferCoefficient = $calc.find(
            '[name="heat-transfer-coefficient"]'
        )), // Коэффициент теплоотдачи внутренней поверхности стенки изолируемого объекта
        ...
        const material = parseInt($material.val(), 10),
            ...
            gasMovingTemperature = parseFloat(
                $gasMovingTemperature.val().replace(/,/, ".")
            ),
            gasThermalConductivity = parseFloat(
                $gasThermalConductivity.val().replace(/,/, ".")
            ),
            gasKinematicViscosity = parseFloat(
                $gasKinematicViscosity.val().replace(/,/, ".")
            ),
            gasThermalDiffusivity = parseFloat(
                $gasThermalDiffusivity.val().replace(/,/, ".")
            ),
            ...
            gasMovingHumidity = parseFloat(
                $gasMovingHumidity.val().replace(/,/, ".")
            ),
            gasPipeOuterDiameter = parseFloat(
                $gasPipeOuterDiameter.val().replace(/,/, ".")
            ),
            gasPipeInnerDiameter = parseFloat(
                $gasPipeInnerDiameter.val().replace(/,/, ".")
            ),
        ...
        if (
            ...
            const diameterInRes =
                $gasPipeType.val() === "rectangular"
                    ? gasPipeInnerDiameter
                    : diameterIn;
            const diameterOutRes =
                $gasPipeType.val() === "rectangular"
                    ? gasPipeOuterDiameter
                    : diameterOut;
