$(function () {
    ...
    $('[name="indoor"]').on("change", function () {
        const $calc = $(".calc");
        const $indoor = $calc.find('[name="indoor"]:checked');
        const $temperature = $calc
            .find('[name="region"] option:selected')
            .data("heat-temperature");
        const $surfaceInsulationTemperature = $calc.find(
            ".surface_insulation_temperature"
        );
        const $pipe = $calc
            .find('[name="pipe"] option:selected')
            .data("material"); // material or no-material
    ...
    $('[name="pipe"]').on("change", function () {
        ...
        const currentPipeMaterial = $(this)
    ...
    $(".surface_insulation_temperature").on("change", function () {
        ...
        const surfaceInsulationTemperature = parseFloat(
        ...
        const errorMessage = () => {
            ...
            if (
                !isIndoor &&
                !isPipeMetal &&
                surfaceInsulationTemperature > 60
            ) {
    ...
    $(".calc_test ._result").on("click", function () {
        ...
        $surfaceInsulationTemperature = $calc.find(
            ".surface_insulation_temperature"
        );
        ...
        const material = parseInt($material.val(), 10),
            ...
            surfaceInsulationTemperature = parseFloat(
                $surfaceInsulationTemperature.val().replace(/,/, ".")
            );
        AeroflexCalc.init();
        ...
        if (
            ...
            let depth = AeroflexCalc.getInsulationDepthWithSurfaceTemperature(
                material,
                diameterIn,
                diameterOut,
                temperatureIn,
